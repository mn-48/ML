...
def link_records(df_youtube, df_spotify):
    match_emails = np.argwhere(np.isin(df_spotify.preferred_contact, df_youtube.preferred_contact)).ravel()
    ...
    fuzzy_matched_ip = np.argwhere(
        np.any(np.isin([i for i in df_spotify.first_three_octets_ip], [j for j in df_youtube.first_three_octets_ip]), axis=1))

    matched_name_and_zip_and_ip = np.intersect1d(fuzzy_matched_ip, matched_name_and_zip)

    all_matches = np.union1d(np.union1d(match_emails, matched_name_and_zip_and_last_four), matched_name_and_zip_and_ip)
